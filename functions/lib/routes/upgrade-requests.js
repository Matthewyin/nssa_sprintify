...
exports.upgradeRequestRoutes = void 0;
const express_1 = require("express");
const admin = __importStar(require("firebase-admin"));
const auth_1 = require("../middleware/auth");
const router = (0, express_1.Router)();
exports.upgradeRequestRoutes = router;
/**
 * 创建升级申请
 */
router.post("/", auth_1.authenticateUser, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            data: Object.assign({ id: docRef.id }, upgradeRequest)
        });
    }
    catch (error) {
...
router.get("/", auth_1.authenticateUser, async (req, res) => {
    try {
        ...
        const userDoc = await admin.firestore()
            ...
            .get();
        const userData = userDoc.data();
        if ((userData === null || userData === void 0 ? void 0 : userData.userType) !== "admin") {
        ...
        let query = admin.firestore().collection("upgradeRequests");
        // 状态筛选
 ...
 * 审批升级申请（管理员）
 */
router.post("/:requestId/review", auth_1.authenticateUser, async (req, res) => {
    try {
        const { uid } = req.user;
 ...
 * 获取用户的升级申请状态
 */
router.get("/my-status", auth_1.authenticateUser, async (req, res) => {
    try {
        const { uid } = req.user;
        // 获取用户最新的申请
        const snapshot = await admin.firestore()
            ...
            .limit(1)
            .get();
        const latestRequest = snapshot.empty ? null : Object.assign({ id: snapshot.docs[0].id }, snapshot.docs[0].data());
        res.json({
            success: true,
            data: {
                latestRequest,
                canApply: !latestRequest || latestRequest.status !== 'pending'
            }
        });
    }
    catch (error) {
...
//# sourceMappingURL=upgrade-requests.js.map