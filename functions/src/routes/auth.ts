 ...
 */
router.post("/upgrade-user", authenticateUser, async (req: AuthenticatedRequest, res) => {
  try {
    const { uid } = req.user!;
    ...
    // 防止修改管理员权限
    if (targetUserData?.userType === "admin") {
      return res.status(400).json({
        success: false,
        error: "不能修改管理员权限"
      });
 ...
 */
router.post("/setup-first-admin", authenticateUser, async (req: AuthenticatedRequest, res) => {
  try {
    const { uid } = req.user!;
