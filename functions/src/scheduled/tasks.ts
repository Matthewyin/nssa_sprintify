 ...
 * 每小时执行一次
 */
export const hourlyTasks = functions
  .region("asia-east1")
  .pubsub
  .schedule("0 * * * *")
  .timeZone("Asia/Shanghai")
 ...
 * 重置每日AI使用次数
 */
async function resetDailyAIUsage(): Promise<void> {
 ...
 * 检查用户活动
 */
async function checkUserActivity(userId: string, date: Date): Promise<boolean> {
 ...
 * 检查过期的冲刺
 */
async function checkExpiredSprints(): Promise<void> {
 ...
 * 更新冲刺进度
 */
async function updateSprintProgress(): Promise<void> {
